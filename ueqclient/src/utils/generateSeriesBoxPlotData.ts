...
export default function generateSeriesBoxPlotData (data: Questionnaire[],  evaluationArea : EvaluationArea ) {
  ...
  const questionnaireDataResult = {
    ...
    questionnaireDataResult.attractiveness.annoying_enjoyable = calculateSummaryStatistics(questionnaireDataList.attractiveness.annoying_enjoyable);
    questionnaireDataResult.attractiveness.bad_good = calculateSummaryStatistics(questionnaireDataList.attractiveness.bad_good);
    questionnaireDataResult.attractiveness.unlikable_pleasing = calculateSummaryStatistics(questionnaireDataList.attractiveness.unlikable_pleasing);
    questionnaireDataResult.attractiveness.unpleasant_pleasant = calculateSummaryStatistics(questionnaireDataList.attractiveness.unpleasant_pleasant);
    questionnaireDataResult.attractiveness.unattractive_attractive = calculateSummaryStatistics(questionnaireDataList.attractiveness.unattractive_attractive);
    questionnaireDataResult.attractiveness.unfriendly_friendly = calculateSummaryStatistics(questionnaireDataList.attractiveness.unfriendly_friendly);

    questionnaireDataResult.novelty.dull_creative = calculateSummaryStatistics(questionnaireDataList.novelty.dull_creative);
    questionnaireDataResult.novelty.usual_leadingEdge = calculateSummaryStatistics(questionnaireDataList.novelty.usual_leadingEdge);
    questionnaireDataResult.novelty.conventional_inventive =calculateSummaryStatistics(questionnaireDataList.novelty.conventional_inventive);
    questionnaireDataResult.novelty.conservative_innovative = calculateSummaryStatistics(questionnaireDataList.novelty.conservative_innovative);

    questionnaireDataResult.efficiency.slow_fast = calculateSummaryStatistics(questionnaireDataList.efficiency.slow_fast);
    questionnaireDataResult.efficiency.inefficient_efficient = calculateSummaryStatistics(questionnaireDataList.efficiency.inefficient_efficient);
    questionnaireDataResult.efficiency.impractical_practical = calculateSummaryStatistics(questionnaireDataList.efficiency.impractical_practical);
    questionnaireDataResult.efficiency.cluttered_organized = calculateSummaryStatistics(questionnaireDataList.efficiency.cluttered_organized);

    questionnaireDataResult.dependability.unpredictable_predictable = calculateSummaryStatistics(questionnaireDataList.dependability.unpredictable_predictable);
    questionnaireDataResult.dependability.notSecure_secure = calculateSummaryStatistics(questionnaireDataList.dependability.notSecure_secure);
    questionnaireDataResult.dependability.doesNotMeetExpectations_meetsExpectations = calculateSummaryStatistics(questionnaireDataList.dependability.doesNotMeetExpectations_meetsExpectations);
    questionnaireDataResult.dependability.obstructive_supportive = calculateSummaryStatistics(questionnaireDataList.dependability.obstructive_supportive);

    questionnaireDataResult.perspecuity.notUnderstandable_understandable = calculateSummaryStatistics(questionnaireDataList.perspecuity.notUnderstandable_understandable);
    questionnaireDataResult.perspecuity.difficultToLearn_easyToLearn = calculateSummaryStatistics(questionnaireDataList.perspecuity.difficultToLearn_easyToLearn);
    questionnaireDataResult.perspecuity.complicated_easy = calculateSummaryStatistics(questionnaireDataList.perspecuity.complicated_easy);
    questionnaireDataResult.perspecuity.confusing_clear =  calculateSummaryStatistics(questionnaireDataList.perspecuity.confusing_clear);

    questionnaireDataResult.stimulation.inferior_valuable =  calculateSummaryStatistics(questionnaireDataList.stimulation.inferior_valuable);
    questionnaireDataResult.stimulation.boring_exciting =  calculateSummaryStatistics(questionnaireDataList.stimulation.boring_exciting);
    questionnaireDataResult.stimulation.notInteresting_interesting =  calculateSummaryStatistics(questionnaireDataList.stimulation.notInteresting_interesting);
    questionnaireDataResult.stimulation.motivating_demotivating =  calculateSummaryStatistics(questionnaireDataList.stimulation.motivating_demotivating);
